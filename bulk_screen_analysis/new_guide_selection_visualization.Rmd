...
raw_regions <- readr::read_delim('regions/merged_windowed_500.bed',delim='\t',col_names=c('Chr','start','end','bins'))
regions <- readr::read_delim('regions/merged_windowed_500.bed',delim='\t',col_names=c('Chr','start','end','bins'))
...
#new bin coordenates
bc <- read_lines('design_bin_coords.txt')
VPR_window_results <- readr::read_delim('sliding_window/num_VPR_pvalues.txt',delim='\t',col_names=c('bin','coeff','pval'))
...
sequence <- str_pad(seq(00001,20000,1),5,side='left',pad=0)
#bins_only$bin <- paste('PHOX2B',sequence,sep='.')
bins_only$bin <- c('PHOX2B.00001',VPR_window_results$bin,'PHOX2B.20000') 
head(bins_only)
###########################
#VPR_bin_results_full <- readr::read_delim('~/Documents/Phox2b/merged/VPR_full_results.txt',delim='\t',col_names=c('Chr','start','end','bin','pvalue','fdr','estimate'))
#bins_only <- dplyr::select(VPR_bin_results_full,Chr,start,end,bin)
VPR_window_results <- readr::read_delim('sliding_window/num_VPR_pvalues.txt',delim='\t',col_names=c('bin','coeff','pval'))
VPR_window_speed <- readr::read_delim('sliding_window/speed_VPR_pvalues.txt',delim='\t',col_names=T)
colnames(VPR_window_speed) <- 'speed'
VPR_window_results$fdr <- p.adjust(VPR_window_results$pval,method='fdr')
VPR_window_results$speed <- VPR_window_speed$speed
...
VPR_guide_results <- readr::read_delim('~/Documents/Phox2b/FINAL/VPR_guide_results.txt',delim='\t',col_names=T)
###############################################################
###############################################################
#loading bin results from the MLM (KRAB)
#KRAB_bin_results_full <- readr::read_delim('~/Documents/Phox2b/merged/VPR_full_results.txt',delim='\t',col_names=c('Chr','start','end','bin','pvalue','fdr','estimate'))
#bins_only <- dplyr::select(KRAB_bin_results_full,Chr,start,end,bin)
KRAB_window_results <- readr::read_delim('sliding_window/num_KRAB_pvalues.txt',delim='\t',col_names=c('bin','coeff','pval'))
KRAB_window_speed <- readr::read_delim('sliding_window/speed_KRAB_pvalues.txt',delim='\t',col_names=T)
colnames(KRAB_window_speed) <- 'speed'
KRAB_window_results$fdr <- p.adjust(KRAB_window_results$pval,method='fdr')
KRAB_window_results$speed <- KRAB_window_speed$speed
KRAB_bin_results <- full_join(regions,left_join(bins_only,KRAB_window_results,by='bin'),by=c('BINS'='bin'),suffix=c('.region','.bin')) %>%
  mutate(no_bins=str_count(bins,'PHOX2B'))
#loading guide results from NHT
KRAB_guide_results <- readr::read_delim('~/Documents/Phox2b/FINAL/KRAB_guide_results.txt',delim='\t',col_names=T)
###############################################################
```
...
VPR <- VPR_counts %>% mutate(V_5=(`VPR_D5-1_S8`+`VPR_D5-2_S9`)/2,V_20=(`VPR_D20_S10`),V_29=(`VPR_D29-1_S11`+`VPR_D29-2_S12`)/2,V_33=(`VPR_D33-1_S13`+`VPR_D33-2_S14`)/2) %>% dplyr::select(GUIDE,Maxi,V_5,V_20,V_29,V_33)

#loading KRAB_counts
KRAB_counts <- read_delim('named_counts/KRAB_table.txt',col_names = T,delim=',')

# taking the mean of the replicates, renaming and selecting useful columns
KRAB <- KRAB_counts %>% mutate(K_5=(`KRAB_D5-1_S1`+`KRAB_D5-2_S2`)/2,K_20=(`KRAB_D20_S3`),K_29=(`KRAB_D29-1_S4`+`KRAB_D29-2_S5`)/2,K_33=(`KRAB_D33-1_S6`+`KRAB_D33-2_S7`)/2) %>% dplyr::select(GUIDE,Maxi,K_5,K_20,K_29,K_33)
...
source('~/Documents/MLM/find_hits.R')
...
#Trying to run the functions on this format of the dataframes. 
p_value <- function(anova){
  anova$`Pr(>Chisq)`[2]
}
model_testing <- function(nested_df){
  nested_df %>% 
    ungroup() %>%
    mutate(mlm = map(data,mlm_function),mm = map(data,min_function)) %>%
    mutate(significance=map2(mlm,mm,anova)) %>%
    mutate(pvalue=map_dbl(significance,p_value)) %>%
    mutate(fdr=p.adjust(pvalue, method="fdr"))%>%
    mutate(tidied=map(mlm,tidy)) %>%
    unnest(tidied) %>%
    filter(term =='Time') %>%
    dplyr::select(-term,-std.error,-statistic,-group)
}
...
VPR_results <- VPR_results %>%
  dplyr::select(-data,-mlm,-mm,-significance)
head(VPR_results)
#KRAB
KRAB_results <- full_KRAB_Dataset %>% 
  ...
  gene_nest()%>%
  model_testing()
KRAB_results <- KRAB_results %>%
  dplyr::select(-data,-mlm,-mm,-significance)
head(KRAB_results)
```

```{r}
#stablishing theme and colors for the plots
...
VPR_results_plot <- mutate(VPR_results,'Result'=ifelse(estimate<0&fdr<0.05,'Depleted',ifelse(estimate>0&fdr<0.05,'Enriched','NoChange')))%>%
  drop_na(Gene)
ggplot(VPR_results_plot,aes(x=estimate,y=-log10(fdr),col=Result))+
...
KRAB_results_plot <- mutate(KRAB_results,'Result'=ifelse(estimate<0&fdr<0.05,'Depleted',ifelse(estimate>0&fdr<0.05,'Enriched','NoChange')))%>%
  drop_na(Gene)
ggplot(KRAB_results_plot,aes(x=estimate,y=-log10(fdr),col=Result))+
  geom_point(alpha=0.7,)+
  xlim(-0.075,0.075)+
  ylim(0,3)+
    theme_classic()+
...
###########################
#pdf('~/Documents/Phox2b/region_images.pdf',width = 6, height = 4)
#Both constructs plot
...
full_regions_results_plot <- rbind(VPR_results_plot,KRAB_results_plot)%>%
  drop_na(Gene)
ggplot(full_regions_results_plot,aes(x=estimate,y=-log10(fdr),col=Result))+
...
#not really used table
library(sjPlot)
sjt.xtab(full_regions_results_plot$Result,full_regions_results_plot$Construct)
regions_table <- as.data.frame(table(full_regions_results_plot$Result,full_regions_results_plot$Construct))
dev.off()
...
ggplot(VPR_window_results_plot,aes(x=coeff,y=-log10(fdr),col=Result))+
  geom_point(alpha=0.5,)+
  xlim(-0.075,0.075)+
  ylim(0,8)+
  theme_classic()+
...
ggplot(KRAB_window_results_plot,aes(x=coeff,y=-log10(fdr),col=Result))+
  geom_point(alpha=0.5,)+
  xlim(-0.075,0.075)+
  ylim(0,2)+
  theme_classic()+
  ...
  ggtitle('KRAB bins')
###########################
#pdf('~/Documents/Phox2b/window_images.pdf',width = 6, height = 4)
#Both constructs
...
ggplot(full_window_results_plot,aes(x=coeff,y=-log10(fdr),col=Result))+
  geom_point(alpha=0.5,)+
  xlim(-0.075,0.075)+
  ylim(0,8)+
  theme_classic()+
...
# Shared bins plot
test <- as.data.frame(table(shared_windows$Result.VPR,shared_windows$Result.KRAB))
colnames(test) <- c('VPR','KRAB','Freq')
g <- ggplot(test,aes(VPR,KRAB))+ 
...
#formattable
library(formattable)
colnames(shared_windows_table)<-c('Bin','VPR','KRAB','Similar')
...
table <- formattable(shared_windows_table,
             align =c("l","c","c","c"),list(
            `VPR`= custom_color_tile('blue','red'),
            `KRAB`= custom_color_tile('blue','red'),
            `Similar`= icon_formatter()
            )
)
...
ggplot(VPR_guide_results_plot,aes(x=logfold,y=-log10(fdr),col=Result))+
...
ggplot(KRAB_guide_results_plot,aes(x=logfold,y=-log10(fdr),col=Result))+
  #geom_point()+
  geom_jitter(alpha=0.5,width=0.5,height=0.5)+
  xlim(-7.5,7.5)+
  #ylim(0,20)+
...
###########################
#pdf('~/Documents/Phox2b/guide_images.pdf',width = 6, height = 4)
#Both plots
...
ggplot(full_guide_results_plot,aes(x=logfold,y=-log10(fdr),col=Result))+
  #geom_point()+
  geom_jitter(alpha=0.5,width=0.5,height=0.5)+
  xlim(-7.5,7.5)+
  #ylim(0,20)+
...
##GUIDES secondary plots, not really used
VPR_guide_results_plot <- mutate(VPR_guide_results,'Result'=ifelse(logfold<0&pvalue<0.05,'Depleted',ifelse(logfold>0&pvalue<0.05,'Enriched','NoChange')))
ggplot(VPR_guide_results_plot,aes(x=logfold,y=-log10(pvalue),col=Result))+
  geom_point(alpha=0.5,)+
  xlim(-7.5,7.5)+
  ylim(0,20)+
  theme_classic()+
  ...
  scale_color_manual(values=mycolors)+
  ggtitle('VPR bins (raw p value)')
KRAB_guide_results_plot <- mutate(KRAB_guide_results,'Result'=ifelse(logfold<0&pvalue<0.05,'Depleted',ifelse(logfold>0&pvalue<0.05,'Enriched','NoChange')))
ggplot(KRAB_guide_results_plot,aes(x=logfold,y=-log10(pvalue),col=Result))+
  geom_point(alpha=0.5,)+
  xlim(-7.5,7.5)+
  ylim(0,20)+
  theme_classic()+
  ...
  scale_color_manual(values=mycolors)+
  ggtitle('KRAB guides (raw p value)')
VPR_guide_results_plot$Construct <- rep('VPR',nrow(VPR_guide_results_plot))
KRAB_guide_results_plot$Construct <- rep('KRAB',nrow(KRAB_guide_results_plot))
full_guide_results_plot <- rbind(VPR_guide_results_plot,KRAB_guide_results_plot)
ggplot(full_guide_results_plot,aes(x=logfold,y=-log10(fdr),col=Result))+
...
#formattable
library(formattable)
colnames(shared_windows_table)<-c('GUIDE','VPR','KRAB','Similar')
...
data_Regions <- dplyr::select(data_Regions,-effect)
...
colnames(full_results)<-c('Region_name','Region_pvalue','Region_fdr','Region_slope','Chr','Region_start','Region_end','Bins_in_region','Bin','Bin_slope','Bin_pvalue','Bin_fdr','Bin_speed','Bin_start','Bin_end')
#writing region only results
write_delim(data_Regions,'regions_results.txt',col_names = TRUE)
#writing region and bin results
write_delim(full_results,'regions_bins_results.txt',col_names = TRUE)
...
#informative column names
correct_colnames_full <- c(colnames(region_bin_guide_Results)[1:15],'guide_start','guide_end','strand','guide_sequence','guide_replicate','guide_name','guide_logfold','guide_pvalue','giude_classification','guide_fdr')
colnames(region_bin_guide_Results)<- correct_colnames_full
# writing out regions+bin+guides results
write_delim(region_bin_guide_Results,'regions_bins_guides_results.txt',delim = '\t',col_names = T)
```

```{r}
source('~/Documents/MLM/classify_hits.R')
time_data <- data.frame('label'=c(0,5,20,29,33),'early'=c(F,T,T,T,T),'late'=c(F,F,T,T,T))

classify_hits <- function(input,time_table){
    input%>%
    add_replicate()%>%
    log_norm()%>%
    slim()%>%
...
#VPR_speed <- classify_hits(full_VPR_Dataset,time_data)
#KRAB_speed <- classify_hits(full_KRAB_Dataset,time_data)
...
#==== colors =====
cold_colors <- c('#204051',
      '#3B6978',
      '#84A9AC',
      '#5F6769')

### Plot
dataTrack <- DataTrack(range = annot,
                                   data = coeffs,
                                   name = "region",
                                   ylim = c(-0.04,0.04),
                                   type = "heatmap",
...
dataTrack2 <- DataTrack(range = annot,
                                   data = significant,
                                   name = "regions",
                                   #ylim = c(0,7),
                                   type = "heatmap",
                                   ...
                                   background.title='black')
dataTrack3 <- DataTrack(range = annot2,
                                   data = coeffs_bins,
                                   name = "bins",
                                   ylim = c(-0.04,0.04),
                                   type = "heatmap",
...
dataTrack4 <- DataTrack(range = annot2,
...
dataTrack_k <- DataTrack(range = annot,
                                   data = df_k$estimate,
                                   name = "    KRAB",
                                   ylim = c(-0.04,0.04),
                                   type = "heatmap",
...
dataTrack2_k <- DataTrack(range = annot,
                                   data = df_k$score,
                                   name = "    KRAB",
                                   ylim = c(0,60),
                                   type = "heatmap",
                                   ...
                                   background.title='#00a7c7')
#mini scale
dataTrack_ns_k_s <- DataTrack(range = annot[df_k$color=='#D2D2D2FF'],
            ...
            col = '#D2D2D2FF',
            background.title='black')
dataTrack_down_k_s <- DataTrack(range = annot[df_k$color=='#6B8993FF'],
            ...
            col = '#6B8993FF',
            background.title='black')
dataTrack_up_k_s <- DataTrack(range = annot[df_k$color=='#E7695DFF'],
...
# additional data
geneTrack <- BiomartGeneRegionTrack(chromosome=chr,
                                    genome=gen,
                                    biomart=biomart,
                                    name='GeneTrack',
                                    background.title='black',
...
superenhancer <- AnnotationTrack(start = sup_df$start, 
                                 width = sup_df$width,
                                 chromosome = chr,
                                 strand = rep("*",2) ,
                                 group = c("Enh1", "Enh2"),
...
regions_gviz <-  regions_gviz %>% mutate('Bonferroni'=p.adjust(pvalue,method='bonferroni')) %>%
...
plot_example <- function(gene,df){
  plot_df<- df%>%add_replicate()%>%log_norm()%>%slim()%>%mutate(Time=as.ordered(Time))
  filter(plot_df,Gene==gene)%>%
    group_by(Replicate)%>%
    mutate(Counts=scale(Counts))%>%
    ggplot(aes(Time,Counts,group=Replicate,col=Replicate))+
    geom_line()+
    ylim(-1.5,1.5)
}
...
top_200_regions <- VPR_results %>% mutate(score=-log10(fdr)) %>% top_n(200,score)
...
full_Results <- full_results %>% mutate(no_bins=str_count(bins,'PHOX2B')) %>%
  filter(fdr<0.05)%>%
  filter(fdr.bin<0.05)
...
best_guide_region <- regions_with_length %>% mutate(best_guides=map2(name,no_bins,get_best_guides)) %>%
unnest() %>%
...
dataTrack5 <- DataTrack(range = annot3,
...
dusa_table <- dplyr::select(best_guide_region,name,Chr,start.region,end.region,fdr,estimate,BINS,speed,GUIDE,NAME,logfold,fdr.guide)
# renaming appropriately 
colnames(dusa_table) <- c('region_name','Chr','start.region','end.region','fdr.region','logfold.region','bin','speed.bin','guide_sequence','guide_name','logfold.guide','fdr.guide')
# examination
head(dusa_table)
# writing it out
...
Data_Track_VPR <- DataTrack(range = mini_annot,
...
Data_Track_KRAB <- DataTrack(range = mini_annot,
...
ggtable(summary_both,'VPR_results','KRAB_results',title = 'Significant regions in both constructs' )
test <- as.data.frame(table(summary_both$VPR_results,summary_both$KRAB_results))
colnames(test) <- c('VPR','KRAB','Freq')
g <- ggplot(test,aes(VPR,KRAB))+ 
  geom_point(aes(size = Freq),color = 'grey')+ 
  theme_classic()+
...
library(formattable)
summary_both <- both %>%
  dplyr::mutate('VPR_results'=ifelse(estimate.VPR<0,-1,1))%>%
  dplyr::mutate('KRAB_results'=ifelse(estimate.KRAB<0,-1,1))%>%
  mutate('Similar'=(sign(VPR_results)==sign(KRAB_results)))%>%
  dplyr::select(Gene,VPR_results,KRAB_results,Similar)


colnames(summary_both)<-c('Region','VPR','KRAB','Similar')
...
table <- formattable(summary_both,
             align =c("l","c","c","c"),list(
            `VPR`= custom_color_tile('blue','red'),
            `KRAB`= custom_color_tile('blue','red'),
            `Similar`= icon_formatter()
...
# for(i in 1:36){
#   cat("### ",KRAB_exclusive_names[i],"\n")
#   plotTracks(list(itrack,dataTrack,dataTrack2,dataTrack3,dataTrack4,dataTrack5,ATACtrack,H3K27ACtrack,PHOX2Btrack,HAND2track,gtrack,geneTrack), transcriptAnnotation = "symbol",collapseTranscripts="longest",from=(KRAB_coordenates$start[i]-500),to=(KRAB_coordenates$end[i]+500),legend=TRUE, cex.title = 0.8, rot.title=1, title.width = 1.5)
#    print(plot_example(KRAB_exclusive_names[i],full_KRAB_Dataset))
#   cat('\n\n')
# }
```
...
best_guide_KRAB_region <- KRAB_regions_with_length %>% mutate(best_guides=map2(name,no_bins,get_best_guides)) %>%
unnest() %>%
dplyr::select(-name1,-no_bins1)
